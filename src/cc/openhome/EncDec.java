...
"$Source: prod/jcprov/src/safenet/jcprov/samples/EncDec.java $" +
        "$Revision: 1.1 $";
     ...
     * to be specific about the private object flag, and CTU_FindObject only
     * lets us specify the object label.
     *
     * @param session
    ...
    {
        String startString = "this is 16 bytes234111111";
        byte[] plainText = null;
		...
		if(startString.getBytes().length %8 != 0) {
			removeByte = 8 - startString.getBytes().length%8;
			plainText = Arrays.copyOf(startString.getBytes(), (startString.getBytes().length / 8 + 1)*8);
		}
		else
			plainText = startString.getBytes();
        ...
        String endString = new String(plainText, 0, (int)lRefDec.value - removeByte);
     ...
     *
     * Typically you would not do this, but rather do a sign/verify operations.
     *
     * @param session
     *  handle to an open session
     *
